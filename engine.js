...
window.PlanEngine=(()=>{
 ...
 function parentIncome(p,year,plan){if(!p)return {amount:0,mode:'未設定'};const firstMode=Object.keys(p.patterns||{})[0]||'働き方1',s=stage(p.stages,year,{mode:firstMode});const mode=s.mode||firstMode;if(mode==='専業主婦'||mode==='無収入')return {amount:0,mode};if(p.usePromotion&&mode===(Object.keys(p.patterns||{})[0]||'働き方1')){const r=(p.promotion||[]).find(x=>n(x.year)===year);if(r){const half=n(r.monthlyPay)*n(r.coefficient)+n(r.performanceBonus)/2;const gross=n(r.monthlyPay)*12+half*(year===n(plan.startYear)?4/3:2);return {amount:gross*n(r.netRate||.72),mode}}}const v=(p.patterns||{})[mode]||{};const override=(p.yearly||{})[String(year)]?.[mode];if(override!==undefined&&override!==null&&override!=='')return {amount:n(override),mode};const start=n(v.startYear)||n(plan.startYear);let anchor=start,base=n(v.base);for(const mark of sorted(v.milestones||[]))if(n(mark.startYear)<=year){anchor=n(mark.startYear);base=n(mark.amount)}const elapsed=Math.max(0,year-anchor);if(v.raiseType==='定額')base+=n(v.raiseAmount)*elapsed;else if(v.raiseType==='定率')base*=Math.pow(1+n(v.raisePercent)/100,elapsed);let amount=v.amountType==='額面'?takeHome(base,plan.taxBrackets):base;return {amount,mode}}
 ...
 const res=[],start=n(plan.startYear),h=Math.max(1,Math.min(80,n(plan.horizon)||40)),nisaCap=36000000;
 let cash=n(plan.initialCash),invest=n(plan.initialInvest),nisaPrincipal=Math.max(0,Math.min(nisaCap,n(plan.initialNisaPrincipal)));
 ...
 for(let i=0;i<h;i++){
  const year=start+i,periodFactor=1;
  ...
  if(plan.retirement&&n(plan.retirement.age)>0&&year-start+n(plan.baseAge)===n(plan.retirement.age)){let payout=n(plan.retirement.payout);if(plan.retirement.usePromotionPayout){const s=promotionSummary(plan.parents?.[0]||{}).find(x=>x.year===year);if(s)payout=s.retirement}eventIncome+=payout}
